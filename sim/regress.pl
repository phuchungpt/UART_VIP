...
sub run_regress {
  ...
  $plusarg =~ s/^\s+//;
  system("make run TESTNAME=$tc SEED=$seed RUNARG=$plusarg"); $end time = time();
$end_time = time();
}
};
    ...
    if($line =~ /^\s*cov\s*=\s*(\w+)/) {
      ...
      if($line =~ /\}/) {$tc_start = 0;}
        ...
        for(my $i = 0; $i < $run_time; $i++) {
          ...
          push @tc_list, int(rand(999999 - 100000+1)) + 1000000;
...
system("grep $fail_key_word_log/*.log >> tmp_report");
system("grep $pass_key_word_log/*.log >> tmp_report"); 
$tc_pass = `grep $pass_key_word_log/*.log | wc -l`; 
$tc_fail = `grep $fail_key_word_log/*.log | wc -l`; 
$tc_unknown = `grep -L -e $pass_key_word -e $fail_key_word_log/*.log | wc -l`; 
